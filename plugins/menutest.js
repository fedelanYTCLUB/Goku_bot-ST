let handler = async (m, {
    ...
    let user = global.db.data.users[userId] // Assuming global.db.data.users exists
    let name = conn.getName(userId)
    let _uptime = process.uptime() * 1000
    let uptime = clockString(_uptime) // Assuming clockString is defined
    let totalreg = Object.keys(global.db.data.users).length // Assuming global.db.data.users exists
    let totalCommands = Object.values(global.plugins).filter((v) => v.help && v.tags).length // Assuming global.plugins exists

    // Assuming botname and moneda are defined elsewhere in your environment
...
❖ *#ttt* ➢ Crea sala de Tres en Raya.
    ...
    // Combine the existing text with the prompt for button selection
    let menuTextWithPrompt = txt + "\n\n🔹 Selecciona una opción:";

    // Assuming _p is defined as your command prefix somewhere accessible here
    const buttons = [{
        buttonId: `${_p}owner`, // Assuming _p is defined
        buttonText: {
            displayText: "👑 Ｃ Ｒ Ｅ Ａ Ｄ Ｏ Ｒ"
        },
        type: 1,
    }, {
        buttonId: `${_p}code`, // Assuming _p is defined
        buttonText: {
            displayText: "🕹 Ｓ Ｅ Ｒ Ｂ Ｏ Ｔ"
        },
        type: 1,
    }, {
        buttonId: `${_p}grupos`, // Assuming _p is defined
        buttonText: {
            displayText: "🌪 Ｇ Ｒ Ｕ Ｐ Ｏ Ｓ"
        },
        type: 1,
    }, ];

    let img = 'https://qu.ax/JznsE.jpg'; // The image URL for the message

    try {
        // Add the reaction before sending the message (optional, but good practice if you want it)
        await m.react('⚽️');

        // Send the message with the image, caption, and buttons
        await conn.sendMessage(
            m.chat, {
                image: {
                    url: img
                }, // Use the image URL
                caption: menuTextWithPrompt, // Use the combined text as the caption
                buttons: buttons, // Attach the defined buttons
                footer: "WHATSAPP BOT X Wirk", // Set the footer text
                viewOnce: true, // Make the message view once
            }, {
                quoted: m
        ...
        throw e; // Rethrow the error after replying
    }
};
...
handler.help = ['menu']
handler.tags = ['main']
handler.command = ['menutest', 'menútest', 'helptest']

export default handler

// Function to format uptime (Keep this function as it's used in txt)
