...
let time = global.db.data.users[m.sender].Subs + 2
if (new Date - global.db.data.users[m.sender].Subs < 2) return conn.reply(m.chat, `${emoji} Debes esperar ${msToTime(time - new Date())} para volver a vincular un *Sub-Bot.*`, m)
